#!/usr/bin/env python3
"""
Tự động tạo Milestone, Labels và Issues chuẩn Enterprise (Đầu vào, Đầu ra, Chi tiết công việc, Tiêu chí nghiệm thu)
cho Sprint 1 trên GitHub Repository.

Cách dùng:
  1. Đăng nhập GitHub CLI: gh auth login -h github.com
     HOẶC đặt biến môi trường: export GITHUB_TOKEN="ghp_xxx"
  2. Chạy: python3 scripts/setup_sprint1_github.py
"""
...
LABELS = [
    {"name": "sprint-1", "color": "0E8A16", "description": "Tasks thuộc Sprint 1: Nền tảng & kết nối API"},
    {"name": "backend", "color": "1D76DB", "description": "Backend FastAPI / Database / Worker"},
    {"name": "frontend", "color": "F9D0C4", "description": "Frontend React / TypeScript"},
    {"name": "devops", "color": "5319E7", "description": "CI/CD / Docker / Infra"},
    {"name": "architecture", "color": "D93F0B", "description": "Cấu trúc & thiết kế hệ thống"},
    {"name": "P0-blocker", "color": "B60205", "description": "Ưu tiên cao nhất - Phải hoàn thành trước"},
    {"name": "P1-high", "color": "D93F0B", "description": "Ưu tiên cao"},
]

MILESTONE = {
    "title": "Sprint 1: Nền tảng backend & kết nối API",
    "description": "Format cấu trúc Monorepo, dựng FastAPI skeleton, PostgreSQL + Alembic, Auth Mock OTP, Geography API và kết nối Frontend.",
...
- [ ] Thư mục gốc không còn file `.json`, `.ts` hay `node_modules` nằm tự do.
- [ ] Chạy `npm --prefix client run build` thành công 100% không có lỗi import.
- [ ] Chạy `npm --prefix client run dev` khởi động dev server mượt mà trên `http://localhost:3000/`.
"""
    },
    {
        "title": "[Sprint 1] [Task 1.1] [BE] Setup FastAPI Application Factory, Config & Health Endpoints",
        "labels": ["sprint-1", "backend", "P0-blocker"],
        "estimate": "2h",
        "body": """## 📌 Tổng quan
...
"""
    },
    {
        ...
        "labels": ["sprint-1", "backend", "devops", "P0-blocker"],
        "estimate": "4h",
        "body": """## 📌 Tổng quan
...
"""
    },
    {
        ...
        "labels": ["sprint-1", "backend", "P1-high"],
        "estimate": "8h",
        "body": """## 📌 Tổng quan
...
"""
    },
    {
        ...
        "labels": ["sprint-1", "devops", "P1-high"],
        "estimate": "4h",
        "body": """## 📌 Tổng quan
...
"""
    },
    {
        ...
        "labels": ["sprint-1", "frontend", "P0-blocker"],
        "estimate": "4h",
        "body": """## 📌 Tổng quan
...
"""
    },
    {
        ...
        "labels": ["sprint-1", "frontend", "P1-high"],
        "estimate": "4h",
        "body": """## 📌 Tổng quan
...
def main():
    print("🚀 Bắt đầu tạo GitHub Issues & Milestone chuẩn Enterprise cho Sprint 1...")
    
    # 1. Check gh auth
    auth_check = run_cmd("gh auth status", check=False)
    if auth_check.returncode != 0 and "GITHUB_TOKEN" not in os.environ:
        print("\n⚠️ Cảnh báo: GitHub CLI chưa được đăng nhập hoặc Token hết hạn.")
        print("💡 Vui lòng chạy lệnh sau để đăng nhập:")
        print("    gh auth login -h github.com")
        print("Hoặc export biến môi trường GITHUB_TOKEN:\n    export GITHUB_TOKEN='ghp_xxx'")
        sys.exit(1)

    # 2. Create Labels
    print("\n🏷️ Đang tạo Labels...")
    for lbl in LABELS:
        run_cmd(f'gh label create "{lbl["name"]}" --repo {REPO} --color "{lbl["color"]}" --description "{lbl["description"]}" --force', check=False)

    # 3. Create Milestone
    print("\n🎯 Đang tạo Milestone...")
    run_cmd(f'gh api repos/{REPO}/milestones -f title="{MILESTONE["title"]}" -f description="{MILESTONE["description"]}" -f due_on="{MILESTONE["due_date"]}"', check=False)

    # 4. Create Issues
    print("\n📝 Đang tạo các Issues chuẩn Enterprise cho Sprint 1...")
    for idx, task in enumerate(TASKS, 1):
        title = task["title"]
        labels = ",".join(task["labels"])
        body = task["body"]
        cmd = f'gh issue create --repo {REPO} --title "{title}" --body "{body}" --label "{labels}" --milestone "{MILESTONE["title"]}"'
        run_cmd(cmd, check=False)

    print("\n🎉 Hoàn tất tạo toàn bộ task cho Sprint 1 trên GitHub với format chuẩn Enterprise!")
